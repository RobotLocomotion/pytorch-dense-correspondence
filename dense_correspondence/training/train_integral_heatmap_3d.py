            ...
            for phase in phases:
                ...
                for i, data in enumerate(loader):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if COMPUTE_SPATIAL_LOSS:
                            ...
                            if COMPUTE_3D_LOSS:
                                # [B, H, W]
                                # uint16
                                depth_img_b = data['data_b']['depth_int16'].to(device).float()

                                # [B, N, H, W]
                                depth_img_b_expand = depth_img_b.unsqueeze(1).expand([-1, N, -1, -1])

                                if verbose:
                                    print("depth_img_b_expand.shape", depth_img_b_expand.shape)


                                # expand to depth_img_b to shape [B, 1, H, W]
                                # depth_gt_b is shape [B, D, N]
                                depth_gt_b = pdc_utils.index_into_batch_image_tensor(depth_img_b.unsqueeze(1), uv_b)

                                # [B, N]
                                depth_gt_b = depth_gt_b.squeeze()

                                if verbose:
                                    print("depth_gt_b.shape", depth_gt_b.shape)


                                # [M,]
                                depth_gt_valid_b = pdc_utils.extract_valid(depth_gt_b,
                                                                           valid)
                                if verbose:
                                    print("depth_gt_valid_b.shape", depth_gt_valid_b.shape)



                                # heatmap_pred_valid [M, H, W]
                                # depth_img_b_valid [M, H, W]
                                depth_img_b_valid = pdc_utils.extract_valid(depth_img_b_expand,
                                                                            valid)

                                if verbose:
                                    print("depth_img_b_valid.shape", depth_img_b_valid.shape)

                                integral_3d_out = predict.get_integral_preds_3d(heatmaps=heatmap_pred_valid,
                                                                                depth_images=depth_img_b_valid,
                                                                                compute_uv=False)

                                depth_pred_b = integral_3d_out['z']
                                if verbose:
                                    print("depth_pred_b.shape", depth_pred_b.shape)
                    ...
                    if i % config['train']['log_per_iter'] == 0:
                        ...
                        log += ', 3D loss: %.1f' %(loss_container['depth'].item()*constants.DEPTH_IM_SCALE)
